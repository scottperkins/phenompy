...
# """Fitted from 1603.08955 - testing purposes"""
# def fitted_hanford_noise( f):
...
#
# names = [ 'aLIGO', 'A+', 'A++', 'Vrt', 'Voyager', 'CE1', 'CE2 wide', 'CE2 narrow', 'ET-B', 'ET-D','LISA']
# fig,ax = plt.subplots()
# # for i in [0,9]:#np.arange(len(noise[1:])):
# #     ax.plot(noise[0],noise[i+1],label = names[i])
#
# ax.plot(np.linspace(10.,2500,1e5),fitted_hanford_noise(np.linspace(10.,2000,1e5)),label='fitted',linestyle='--')
# # ax.plot(noise_lisa[0],noise_lisa[1],label='LISA')
# # ax.plot(noise_lisa[0],noise_lisa[2],label='Also LISA?')
# ax.set_xscale('log')
# ax.set_yscale('log')
...
class IMRPhenomD():
    ...
    def __init__(self, mass1, mass2,spin1,spin2, collision_time, \
        ...
        self.Z =Distance(Luminosity_Distance/mpc,unit=u.Mpc).compute_z(cosmology = self.cosmo_model)#self.calculate_Z(self.DL)
    ...
    ###########################################################################################################
    """Calculates the parameters beta0, apha0, beta1, and alpha1 based on the condition that the phase is continuous
    across the boundary"""
    ...
    def calculate_fisher_matrix(self,detector,int_scheme = 'simps',stepsize=None):
        ...
        #         self.noise_func = CubicSpline(noise[0],self.noise_curve)
            ...
            self.noise_curve = self.sym_noise_curve(np.asarray(freq))
            self.noise_func = self.sym_noise_curve
        elif detector == 'aLIGOFitted':
            if stepsize != None:
                freq = np.arange(1,10000,stepsize)
            self.noise_curve = self.fitted_hanford_noise(np.asarray(freq))
            self.noise_func = self.fitted_hanford_noise
        else:
    ...
    def populate_noise(self, detector='aLIGO',int_scheme='simps',stepsize = None):
        ...
        if detector in names:
            ...
            if stepsize != None:
                freq = np.arange(1,10000,stepsize)
            noise_curve = self.sym_noise_curve(np.asarray(freq))
            noise_func = self.sym_noise_curve
        elif detector == 'aLIGOFitted':
            if stepsize != None:
                freq = np.arange(1,10000,stepsize)
            noise_curve = self.fitted_hanford_noise(np.asarray(freq))
            noise_func = self.fitted_hanford_noise
...
# """Child class of IMRPhenomD - adds modification to the phase of the waveform in the inspiral region -
...
# class Modified_IMRPhenomD(IMRPhenomD):
#     def __init__(self, mass1, mass2,spin1,spin2, collision_time,
...
#         - symmratio*( beta1*f1*M + beta2*np.log(M*f1) - \
...
#         #(alpha1*f2*M -alpha2*(1/(f2*M)) + \
#         #(4/3)*alpha3*(f2*M)**(3/4) + alpha4*np.arctan((f2-alpha5*fRD)/(fdamp))+phase_mod*(chirpm*np.pi*f2)**(self.bppe/3))
#     ######################################################################################
...
#         return (super(Modified_IMRPhenomD,self).phi_ins(f,phic,tc,chirpm,symmratio,delta,chi_a,chi_s,sigma2,sigma3,sigma4,pn_phase)+phase_mod*(chirpm*np.pi*f)**(self.bppe/3))
...
#         return super(Modified_IMRPhenomD,self).amp_ins_vector(f,A0,phic,tc,chirpm,symmratio,chi_s,chi_a)
#     def amp_int_vector(self,f,A0,phic,tc,chirpm,symmratio,chi_s,chi_a,phase_mod):
#         return super(Modified_IMRPhenomD,self).amp_int_vector(f,A0,phic,tc,chirpm,symmratio,chi_s,chi_a)
#     def amp_mr_vector(self,f,A0,phic,tc,chirpm,symmratio,chi_s,chi_a,phase_mod):
#         return super(Modified_IMRPhenomD,self).amp_mr_vector(f,A0,phic,tc,chirpm,symmratio,chi_s,chi_a)
#     ######################################################################################
#
...
#         # for Rv in Rvs:
#         #     # Z1 = Distance(Rv/mpc,unit=u.Mpc).compute_z(cosmology = self.cosmo_model)
#         #     # Z2 = Distance((self.DL-Rv)/mpc,unit=u.Mpc).compute_z(cosmology =self.cosmo_model)
#         #     Z1 = Zfunc(Rv/mpc)
#         #     Z2 = Zfunc((self.DL-Rv)/mpc)
#         #     D = (1+self.Z)*(integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z2 )[0]\
#         #     - integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z1 )[0])
#         #     lambdas.append((D*np.pi**2*self.chirpm/((1+self.Z)*delta_beta))**(1/2))
#         # return lambdas
#
...
#         # for Rv in Rvs:
#         #     # Z1 = Distance(Rv/mpc,unit=u.Mpc).compute_z(cosmology = self.cosmo_model)
#         #     # Z2 = Distance((DL_GW170817-Rv)/mpc,unit=u.Mpc).compute_z(cosmology =self.cosmo_model)
#         #     Z1 = Zfunc(Rv/mpc)
#         #     Z2 = Zfunc((DL_GW170817-Rv)/mpc)
#         #     D = (1+Z)*(integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z2 )[0]\
#         #      - integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z1 )[0])
#         #     lambdas.append((2*Delta_T_measured*fpeak_measured**2/((1+Z)*D))**(-1/2))
#         # return lambdas
#
...
#         # for Rv in Rvs:
#         #     # Z1 = Distance(Rv/mpc,unit=u.Mpc).compute_z(cosmology = self.cosmo_model)
#         #     # Z2 = Distance((GW15DL-Rv)/mpc,unit=u.Mpc).compute_z(cosmology =self.cosmo_model)
#         #     Z1 = Zfunc(Rv/mpc)
#         #     Z2 = Zfunc((GW15DL-Rv)/mpc)
#         #     D = (1+GW15Z)*(integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z2 )[0]\
#         #      - integrate.quad(lambda x: 1/(H0*(1+x)**2*np.sqrt(.3*(1+x)**3 + .7)),0,Z1 )[0])
#         #     lambdas.append((D*np.pi**2*GW15chirpm/((1+GW15Z)*GW15DeltaBeta))**(1/2))
#         # return lambdas
#
...
#
#         points = 5000
#         x,y = self.create_degeneracy_data(self,delta_beta = None,points = points)
